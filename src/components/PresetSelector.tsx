...
export default function PresetSelector({ recipe, onChange }: Props) {
  ...
  const [platformFilter, setPlatformFilter] = useState<"all" | "short" | "instagram" | "youtube" | "twitter">("all");
...
const handleHeightChange = useCallback((h: number) => {
  ...
  const filteredPresets = PRESETS.filter((p) => {
    ...
    if (platformFilter === "short") return p.platform.toLowerCase().includes("short") || p.platform.toLowerCase().includes("tiktok") || p.platform.toLowerCase().includes("reels");
    if (platformFilter === "instagram") return p.platform.toLowerCase().includes("instagram") || p.platform.toLowerCase().includes("ig");
    if (platformFilter === "youtube") return p.platform.toLowerCase().includes("youtube");
    return p.platform.toLowerCase().includes("twitter") || p.platform.toLowerCase().includes("x");
  });
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-xl border border-outline-variant/20 bg-gradient-to-b from-surface-container-high/50 to-surface-container-low/30 p-3">
        <p className="text-[10px] uppercase tracking-[0.18em] font-semibold text-on-surface-variant">Output Size</p>
        <p className="text-xs text-on-surface-variant mt-1">Select the best canvas for each platform with smart aspect presets.</p>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {[
          ...
          <button
            ...
            className={cn(
              "px-2.5 py-1.5 rounded-full text-[10px] font-semibold uppercase tracking-wider border transition-all",
              platformFilter === item.id
                ? "border-primary/40 bg-primary/10 text-primary"
                : "border-outline-variant/20 bg-surface-container-low text-on-surface-variant hover:border-outline-variant/35"
            )}
          >
            {item.label}
          </button>
          ...
          filteredPresets.map((preset) => {
            ...
            return (
              <button
                ...
                aria-pressed={active}
                className={cn(
                  "min-h-[44px] min-w-[44px] flex items-center gap-2 p-2.5 rounded-xl border text-left transition-all duration-150 cursor-pointer hover:scale-[1.02] active:scale-[0.98]",
                  active
                    ? "border-primary/45 bg-primary/10"
                    : "border-[var(--border)] bg-[var(--surface)] hover:border-primary/30 hover:bg-surface-container-low"
                )}
              >
                ...
                <div className="min-w-0 flex-1 overflow-hidden">
                  ...
                  <p className="text-[9px] text-on-surface-variant/60 font-mono mt-1 truncate">
